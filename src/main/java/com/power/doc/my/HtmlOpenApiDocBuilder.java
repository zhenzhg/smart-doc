 ...
 * @author zzg
 */
public class HtmlOpenApiDocBuilder {
    ...
    public static void buildApiDoc(ApiConfig config) {
        config.setOutPath(new File(config.getOutPath(), ".." + FILE_SEPARATOR + "openapidoc").getPath());
        JavaProjectBuilder javaProjectBuilder = new JavaProjectBuilder();
        ApiInterfaceDocBuilder.exPatternPath(config);
    ...
    public static void buildApiDoc(ApiConfig config, JavaProjectBuilder javaProjectBuilder) {
        ...
        IDocBuildTemplate docBuildTemplate = new OpenApiDocBuildTemplate();
        ...
        if (config.isAllInOne()) {
            ...
            buildJsClient(apiDocList, config.getOutPath());
        }
    }
    ...
    private static void buildJsClient(List<ApiDoc> apiDocList, String outPath) {
        FileUtil.mkdirs(outPath);
        String strTime = DateTimeUtil.long2Str(now, DateTimeUtil.DATE_FORMAT_SECOND);
        for (ApiDoc doc : apiDocList) {
            Template apiTemplate = BeetlTemplateUtil.getByName("jsclient.btl");
            apiTemplate.binding(TemplateVariable.NAME.getVariable(), doc.getName());//类名
            apiTemplate.binding(TemplateVariable.LIST.getVariable(), doc.getList());
            apiTemplate.binding(TemplateVariable.VERSION.getVariable(), now);

            FileUtil.nioWriteFile(apiTemplate.render(), outPath + FILE_SEPARATOR + doc.getAlias() + ".js");
        }
    }
