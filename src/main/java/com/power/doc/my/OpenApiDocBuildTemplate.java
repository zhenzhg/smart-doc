...
import static com.power.doc.constants.DocGlobalConstants.MULTIPART_TYPE;
 ...
 */
public class OpenApiDocBuildTemplate implements IDocBuildTemplate<ApiDoc> {
...
//                continue;
//            }
            if (!cls.getFullyQualifiedName().matches("^com\\.x\\.mymall\\.openapi\\.web\\.controller\\..*$") || !hasOpenApiAnnotation(cls) || StringUtil.isNotEmpty(ignoreTag)) {
                continue;
            }
    ...
    private boolean hasOpenApiAnnotation(JavaClass javaClass) {
        List<JavaAnnotation> annotations = javaClass.getAnnotations();
        for (JavaAnnotation annotation : annotations) {
            if (annotation.getType().getName().equals("com.x.mymall.core.server.web.annotation.OpenApi")) {
                return true;
            }
        }
    ...
    private JavaAnnotation getOpenApiMethodAnnotation(JavaMethod javaMethod) {
        List<JavaAnnotation> annotations = javaMethod.getAnnotations();
        for (JavaAnnotation annotation : annotations) {
            if (annotation.getType().getName().equals("com.x.mymall.core.server.web.annotation.OpenApiMethod")) {
                return annotation;
            }
        }
    ...
    private String getOpenApiMethodName(JavaMethod javaMethod) {
        List<JavaAnnotation> annotations = javaMethod.getAnnotations();
        for (JavaAnnotation annotation : annotations) {
            if (annotation.getType().getName().equals("com.x.mymall.core.server.web.annotation.OpenApiMethod")) {
                return annotation.getNamedParameter("method").toString();
            }
        }
    ...
    private List<ApiMethodDoc> buildControllerMethod(final JavaClass cls, ApiConfig apiConfig, ProjectDocConfigBuilder projectBuilder) {
        ...
        for (JavaMethod method : methods) {
            ...
            JavaAnnotation javaAnnotation = getOpenApiMethodAnnotation(method);
...
//            }
            ...
            RequestMapping requestMapping = new OpenApiRequestMappingHandler()
            ...
            if (Objects.nonNull(requestMapping)) {
                ...
                apiMethodDoc.setResponseUsage(JsonBuildHelper.buildReturnJson(method, projectBuilder, true));
    ...
    private ApiRequestExample buildReqJson(JavaMethod method, ApiMethodDoc apiMethodDoc, String methodType,
        ...
        apiMethodDoc.setContentType("application/x-www-form-urlencoded;charset=utf-8");
        ...
        if (parameterList.size() > 0) {
            ...
            List<ApiParam> defParams = getDefaultParams(method);
            for (ApiParam apiParam : defParams) {
                String mockValue = apiParam.getField().equals("method") ? apiParam.getDesc() : RandomUtil.randomValueByType("String");
                jsonBody.append(DocUtil.handleJsonStr(apiParam.getField()))
                        .append(":")
                        .append(mockValue);
                jsonBody.append(",");
                ...
                formData.setDesc(apiParam.getDesc());
                formData.setType("text");

                try {
                    formData.setValue(java.net.URLEncoder.encode(mockValue, Charset.DEFAULT_CHARSET));
            ...
            for (int i = 0; i < parameterList.size(); i++) {
                ...
                String mockValue = RandomUtil.randomValueByType(simpleTypeName);
                ...
                formData.setDesc(parameter.getComment());
                formData.setType("text");
                try {
                    formData.setValue(java.net.URLEncoder.encode(mockValue, Charset.DEFAULT_CHARSET));
                } catch (UnsupportedEncodingException e) {
...
//            );
            ...
            String exampleBody = UrlUtil.urlJoin(DocGlobalConstants.EMPTY, DocUtil.formDataToMap(formDataList))
                    .replace("?", DocGlobalConstants.EMPTY);
            exampleBody = StringUtil.removeQuotes(exampleBody);

            exampleBody = String.format("curl -X %s -i %s  --data \n'%s'", methodType, url, exampleBody);
...
//                                + javaMethod.getName() + "Check it in " + javaMethod.getDeclaringClass().getCanonicalName());
        ...
        List<ApiParam> defList = getDefaultParams(javaMethod);
        paramList.addAll(0, defList);

        return paramList;
    }

    private List<ApiParam> getDefaultParams(JavaMethod javaMethod) {
        List<ApiParam> defList = new ArrayList<>();
        String apiMethodName = getOpenApiMethodName(javaMethod);
        if (apiMethodName != null) {
            apiMethodName = apiMethodName.replace("\"", "")
                    .trim();
        }

        ApiParam apiParam = new ApiParam();
        apiParam.setField("method");
        apiParam.setType("string");
        apiParam.setRequired(true);
        apiParam.setDesc(apiMethodName).setVersion(DocGlobalConstants.DEFAULT_VERSION);
        defList.add(apiParam);

        apiParam = new ApiParam();
        apiParam.setField("appId");
        apiParam.setType("string");
        apiParam.setRequired(true);
        apiParam.setDesc("appId").setVersion(DocGlobalConstants.DEFAULT_VERSION);
        defList.add(apiParam);

        apiParam = new ApiParam();
        apiParam.setField("timestamp");
        apiParam.setType("string");
        apiParam.setRequired(true);
